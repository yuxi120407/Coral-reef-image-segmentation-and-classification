...
def newtxt(path_txt,path_image,crop_length,crop_width):
    ...
    for m in range(count_points):
        ...
        if ((new_l=="Agalg") or (new_l=="Aga") or (new_l=="Agalf") or (new_l=="Col") or (new_l=="Helc") or (new_l=="Mdrc") or (new_l=="Mdrcb") or (new_l=="Mdrsd")or (new_l=="Mdrsf") or (new_l=="Man")or (new_l=="Mon")or (new_l=="Ocu")or (new_l=="Sco")or (new_l=="Sol")or (new_l=="Ste")or (new_l=="STY")):
            label_encode[m] = 0
        elif (new_l=="DCP"):
            label_encode[m] = 1
        elif (new_l=="ROC"):
    ...
    with open(str('2015new/')+name_txt+str('.txt'), 'w+') as newtxtfile:
...
def newimagedata(crop_length,crop_width):
    ...
    read_files = glob.glob("./2015-image-data/*.txt")
    #length = len(read_files)
    for name in read_files:
        name = name.split("\\")[1]
        name = name.split(".")[0]
        path_txt = str('./2015-image-data/')+name+str('.txt')
        path_image = str('./2015-image/')+name+str('.jpg')
